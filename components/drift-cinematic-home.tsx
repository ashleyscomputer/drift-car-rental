...
const G_WAGON_ANGLES = [
  '/vehicles/gallery/g-wagon/1.jpg',
  '/vehicles/gallery/g-wagon/2.jpg',
  '/vehicles/gallery/g-wagon/3.jpg',
];

const SLS_CLOSED = 'https://commons.wikimedia.org/wiki/Special:Redirect/file/2010%20Mercedes-Benz%20SLS%20AMG%20(C%20197)%20Blackbird%20coupe%20(2010-10-16)%2002.jpg?width=2560';
const SLS_OPEN = 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d6/Matte_Black_Mercedes_SLS_With_Doors_Up_%2812157179396%29.jpg/2560px-Matte_Black_Mercedes_SLS_With_Doors_Up_%2812157179396%29.jpg';
...
export default function DriftCinematicHome() {
  ...
  const gCarX = reducedMotion ? 0 : mix(-36, 66, range(gProgress, 0.12, 0.76)) + pointer.x * 7;
  const gCarY = reducedMotion ? 0 : mix(34, -8, range(gProgress, 0.08, 0.74)) + pointer.y * 7;
  const gCarRotate = reducedMotion ? 0 : mix(-0.45, 0.55, range(gProgress, 0.18, 0.78));
  const gLightX = reducedMotion ? 0 : mix(-52, 108, gProgress) + pointer.x * 14;
  ...
  const slsX = reducedMotion ? 0 : mix(28, -26, range(slsProgress, 0.06, 0.72)) + pointer.x * 5;
  const slsY = reducedMotion ? 0 : mix(26, -4, range(slsProgress, 0.08, 0.72)) + pointer.y * 5;
      ...
      <div className="sticky top-0 isolate h-[100svh] overflow-hidden bg-[#050607]">
        ...
        <div className="absolute inset-0" style={{ opacity: gOpacity }} aria-hidden={gOpacity < 0.02}>
          ...
          <div
            ...
            style={{ opacity: gWordmarkOpacity, transform: `translate3d(calc(-50% + ${reducedMotion ? 0 : mix(-10, 20, gProgress) + pointer.x * 4}px), -50%, 0)` }}
          >
            DRIFT
          </div>
          ...
          >
            ...
            {G_WAGON_ANGLES.map((src, index) => (
              <img
                ...
                className="absolute inset-0 h-full w-full object-cover object-center [filter:saturate(.68)_contrast(1.13)_brightness(.7)] [mask-image:linear-gradient(90deg,transparent_0%,rgba(0,0,0,.82)_8%,black_23%,black_92%,transparent_100%)] max-md:object-[52%_center]"
                style={{ opacity: gImageOpacities[index], transition: reducedMotion ? 'opacity 240ms linear' : 'opacity 120ms linear' }}
                onError={(event) => {
                  const image = event.currentTarget;
                  image.onerror = null;
                  image.src = '/vehicles/g-wagon.jpg';
                }}
              />
            ))}
        ...
        <div className="absolute inset-0" style={{ opacity: slsOpacity }} aria-hidden={slsOpacity < 0.02}>
          ...
          <div
            ...
            style={{ opacity: 0.055 * slsOpacity, transform: `translate3d(calc(-50% + ${reducedMotion ? 0 : pointer.x * 3}px), -50%, 0)` }}
          >
            GULLWING
          </div>
          ...
          >
            ...
            <img
              src={SLS_CLOSED}
              alt="Mercedes-Benz SLS AMG cinematic showcase with gullwing doors closed"
              loading="lazy"
              decoding="async"
              className="absolute inset-0 h-full w-full object-cover object-center [filter:saturate(.62)_contrast(1.13)_brightness(.64)] [mask-image:linear-gradient(90deg,transparent_1%,rgba(0,0,0,.9)_10%,black_23%,black_91%,transparent_99%)] max-md:object-[52%_center]"
              style={{ opacity: slsClosedOpacity }}
            />
            <img
              src={SLS_OPEN}
              alt="Mercedes-Benz SLS AMG cinematic showcase with gullwing doors open"
              loading="lazy"
              decoding="async"
              className="absolute inset-0 h-full w-full object-cover object-center [filter:saturate(.64)_contrast(1.15)_brightness(.68)] [mask-image:linear-gradient(90deg,transparent_1%,rgba(0,0,0,.9)_10%,black_23%,black_91%,transparent_99%)] max-md:object-[50%_center]"
              style={{ opacity: slsOpenOpacity }}
            />
        ...
        >
          ...
          <a href="#browse" className="mt-6 inline-flex min-h-12 items-center gap-2 rounded-full bg-black px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black">
